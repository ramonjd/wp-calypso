...
import { getAPIShippingZones } from 'woocommerce/state/wc-api/shipping-zones/selectors';
import { LOADING } from 'woocommerce/state/wc-api/reducer';

const getAPIShippingZoneMethods = ( state, siteId = getSelectedSiteId( state ) ) => {
	return get( state, [ 'extensions', 'woocommerce', 'wcApi', siteId, 'shippingZoneMethods' ] );
};
